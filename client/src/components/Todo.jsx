      ...
      >
        {
          data.reverse().map(item=><div onClick={()=>handleCardClick(item)} key={item._id} className={`w-full  hover:outline-1  ${day? "bg-white hover:shadow-[0_6px_12px_-1px_rgba(0,0,0,0.4)] text-black hover:outline-black":"bg-gray-600 text-white hover:outline-white hover:shadow-[0_6px_12px_-1px_rgba(256,256,256,0.4)]"}  flex flex-col shadow-[0_1px_6px_-1px_rgba(0,0,0,0.4)] rounded-xl  `}>
            ...
            <div className={`flex flex-row-reverse items-center px-4 py-2  ${item.status == "inProgress" ? "bg-yellow-500 text-black ": item.status == "finished" ? "bg-green-600 text-white" : "bg-gray-100 text-black"  } gap-2 rounded-xl`}>
              <div className={`w-15 h-10   ${item.status == "finished"? "hover:bg-green-800":"hover:bg-white" } rounded-full flex items-center justify-evenly hover:outline-1`} onClick={(e)=>{
                 e.stopPropagation()
              ...
              { item.status !="finished" &&   <div className={`w-20 h-10  ${item.status == "finished"? "hover:bg-green-800":"hover:bg-white" } rounded-2xl flex items-center justify-evenly hover:outline-1`} onClick={(e)=>{
                 e.stopPropagation()
              ...
              { item.status !="todo" &&   <div className={`w-20 h-10 rounded-2xl  ${item.status == "finished"? "hover:bg-green-800":"hover:bg-white" } flex items-center justify-evenly hover:outline-1`} onClick={(e)=>{
                 e.stopPropagation()
              ...
              className={`w-20 h-10  rounded-full  ${item.status == "finished"? "hover:bg-green-800":"hover:bg-white" } flex items-center justify-evenly hover:outline-1`}>
                <p>Delete</p>
                <Trash size={20} />
              </div>
            </div>
